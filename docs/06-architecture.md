...
Если объём вырастет — миграция на Room сводится к замене `GameStore` при том же интерфейсе.

**Как устроено.** Типизированный `DataStore<T>` (артефакт `androidx.datastore:datastore`) с JSON-сериализатором на
`kotlinx.serialization`, не protobuf. Слои:
...
- `GameStore` — порт ввода-вывода в `data` (`load`, `save` целым снимком): `InMemoryGameStore` и `DataStoreGameStore`.
  Отдельный порт нужен, чтобы репозиторий проверялся без диска и с подделанными сбоями.
- Формат файла — отдельные DTO (`GameSaveFile`, `GameSnapshotDto`…) и мапперы, а не доменные модели: домен не знает о
...
(прежний файл цел: запись атомарна). Повреждённый файл или чужая версия: обработчик заменяет файл пустым
сохранением, а `load()` один раз возвращает `CORRUPTED` или `INCOMPATIBLE_VERSION`, чтобы игрок не терял игру молча.
Версия читается первой и отдельно, иначе смена формата выглядела бы как повреждение.

Настройки лежат в отдельном файле (`DataStoreSettingsRepository`): сброс игры при смене формата не должен трогать звук
и подсказки. Их сбои мягкие: повреждённый файл или неудачное чтение дают значения по умолчанию, сбой записи —
`WRITE_FAILED`. Создавать больше одного DataStore на файл в процессе нельзя (в Koin — `single`).

**Подключение.** `storageModule` (Koin, требует Context) создаёт `DataStoreGameStore` (`game.json`) и
`DataStoreSettingsRepository` (`settings.json`) на общей области ввода-вывода; `appModule` берёт `GameStore` оттуда.
В JVM-тесте `storageModule` подменяется хранилищами в памяти. При запуске `PetCreationViewModel` вызывает
`game.load()`: есть игра → сразу главный экран; нет → создание питомца; `CORRUPTED`/`INCOMPATIBLE_VERSION` → создание
с пояснением (сохранение уже сброшено); `READ_FAILED` → экран с кнопкой «Повторить», новую игру поверх не начинаем.
Сбой записи при создании питомца показывается на форме. Остальные ViewModel'и (магазин, копилка, задания, итог
недели, настройки) пока игнорируют `Error` (`// TODO(хранилище)`): игра не применяет команду, но ребёнку об этом не
говорят. Файлы лежат в `files/datastore/` и попадают под Auto Backup: после переустановки сохранение может вернуться.

Версия формата (`GAME_SAVE_VERSION`) лежит в самом файле, а не в `GameState`: при несовпадении сохранение
сбрасывается, а `load()` сообщает `INCOMPATIBLE_VERSION` (для хакатона достаточно).
...
тоже в `domain`; оба хранятся на диске (см. «Хранение»), поэтому команды и запись настроек возвращают `Result` с
`StorageError` (`domain/util/result`, `domain/storage`).
...
питомца и над `NavDisplay`, а не внутри `Home`. Вид и движок создаются один раз, сразу после первого кадра, пока
пользователь выбирает питомца (~0.3 с на главном потоке), и тогда же прогреваются моделью-заглушкой
(`PetWarmUpSpec`), чтобы первый показ не ждал шейдеров. Дальше вид только подгружает нужную модель (смена окраса
лишь перекрашивает материал), встаёт на слот главного экрана (`PetHostState`) и прячется. Ограничения, на которых
это держится:

- Прятать вид нужно сдвигом за экран и паузой отрисовки, а не через `visibility`: невидимый SurfaceView
  теряет поверхность, а создавать её заново дорого. Сдвиг заодно убирает вид из hit-теста Compose.
- Размер SurfaceView менять нельзя: после смены размера Filament продолжает рисовать в старый буфер, система его
  отбрасывает (`BLASTBufferQueue: rejecting buffer`), и питомец пропадает. Поэтому буфер фиксированный (квадрат
  1024 px, `setFixedSize`), слот на главном экране квадратный, а система масштабирует буфер под размер вида.
- SurfaceView не следует за анимацией Compose, поэтому переходы между экранами не двигают содержимое: только
  затухание 150 мс (`MainNavigation`). Питомец показывается, как только Home становится целью перехода, и
  скрывается, как только перестаёт ею быть. Стандартные 0.7 с перехода Navigation 3 держали кнопки неактивными.
- `Engine` привязан к потоку, который его создал, поэтому создавать движок в фоне нельзя.
- Версия Filament закреплена на 1.71.5: на 1.76.1 не работает взмах руки по нажатию (см. `libs.versions.toml`).

Пока граф начинается с `Home`; выбор питомца идёт до него (`PetCreationScreen`: нет питомца →
